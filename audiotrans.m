...
conf.f_spacing          = 5;
conf.nsubcarriers       = 300;
if mod(conf.nsubcarriers,2) ~= 0
   disp('WARNING: Number of subcarriers should be even.'); 
end
conf.f_sampling         = 48000; 
conf.os_factor          = conf.f_sampling / conf.nsubcarriers / conf.f_spacing;
assert(mod(conf.os_factor, 1) == 0, 'over-sampling factor should be integer');
conf.f_mixer            = 2000;
conf.nbits              = 10000;    % number of bits ? per frame?
...
%     title('Received Signal')
    ...
    [rxbits, rxbits_pt, conf]       = rxtest(rxsignal, rawtxsignal(:, 2),conf);
    
    res.rxnbits(k)      = length(rxbits);  
    res.biterrors(k)    = sum(rxbits ~= txbits);
    
    res.rxnbits_pt(k)      = length(rxbits_pt);  
    res.biterrors_pt(k)    = sum(rxbits_pt ~= txbits);
...
per_pt = sum(res.biterrors_pt > 0)/conf.nframes;
ber_pt = sum(res.biterrors_pt)/sum(res.rxnbits_pt)
